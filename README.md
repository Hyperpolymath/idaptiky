...
- 🧩 **Logic puzzles** - Solve puzzles by forward and backward execution
- ↩️ **Undo systems** - Build robust undo/redo functionality
- 🔬 **Reversible algorithms** - Explore reversible computing concepts
- 🎓 **Education** - Learn about time-reversible computation
...
|-------------|--------|-----------|---------|---------|
| **ADD** | `ADD a b` | `a = a + b` | `a = a - b` | `ADD x y` → x=15 (if x=10, y=5) |
| **SUB** | `SUB a b` | `a = a - b` | `a = a + b` | `SUB x y` → x=5 (if x=10, y=5) |
| **SWAP** | `SWAP a b` | swap values | swap again | `SWAP x y` → x=5, y=10 (if x=10, y=5) |
| **NEGATE** | `NEGATE a` | `a = -a` | negate again | `NEGATE x` → x=-10 (if x=10) |
| **NOOP** | `NOOP` | (nothing) | (nothing) | `NOOP` → no change |
...
├── src/
│   ├── core/
│   │   ├── Instruction.res      # Instruction interface
...
├── .gitignore                   # Updated for ReScript + Deno
├── MIGRATION-GUIDE.md          # Migration documentation
├── CONVERSION-SUMMARY.md       # Conversion report
...
[SUCCESS] ✓ ADD test passed
[SUCCESS] ✓ ADD invert test passed
[SUCCESS] ✓ SWAP test passed
[SUCCESS] ✓ SWAP invert test passed
[SUCCESS] All tests passed!
```

Tests verify:
- ✓ Forward execution correctness
- ✓ Inverse execution correctness
- ✓ Perfect reversibility (forward then inverse = identity)
...
## 📖 Reversible Computing Background
...
- ➕ Provenance tracking encouraged
...
**Built with ❤️ using ReScript + Deno**
